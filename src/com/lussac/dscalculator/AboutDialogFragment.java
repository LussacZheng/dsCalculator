...
class AboutDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		builder.setTitle("屌丝计算器 v0.3.1");
		builder.setMessage("应用介绍:\n\n"
				+ "1. 此计算器对四则运算表达式采用 逆波兰式(Reverse Polish notation) 算法。同时为了保证结果的精确度，对所有数字进行分数化处理。故当 小数部分过长整数过大 或 小数部分过长 时，可能出现 结果异常 的情况。\n\n"
...
class AboutUpdateDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		AlertDialog.Builder builder = new AlertDialog.Builder(getActivity());
		builder.setTitle("更新说明(v0.3.1)");
		builder.setMessage("1. 实现了屏幕自适应；\n"
				+ "2. 添加了侧滑菜单，并实现划动手势响应；\n"
				+ "3. 功能实验室增加了随机整数生成器；\n"
				...
				+ ".1+.2(.3-.).4×.(5.÷.6).\n"
				+ "补全为:\n"
				+ "0.1+0.2×(0.3-0.0)×0.4×0.0×(5.0÷0.6)×0.0；\n"
				+ "6. 解决了某些已知关于小数的bug；\n"
...
class HelpDialogFragment extends DialogFragment {
	...
	public Dialog onCreateDialog(Bundle savedInstanceState) {
		...
		builder.setMessage("1. 长按C可恢复意外清除的算式；\n"
				...
				+ "3. 随机数生成：\n输入区间[a,b]，点击按钮随机生成一个区间内的整数（区间左端点默认为1，可修改）。");
		return builder.create();
	}
}
